...
TEST_CASE("ghost gets eaten", "[task_1]") { REQUIRE(can_eat_ghost(true, true)); }
...
TEST_CASE("ghost does not get eaten because no power pellet is active, even if not "
          "touching ghost",
          "[task_1]") {
...
TEST_CASE("win if all dots eaten", "[task_4]") { REQUIRE(won(true, false, false)); }
